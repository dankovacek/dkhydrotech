...
KEY_DIR = os.path.join(BASE_DIR, 'mainsite/tmp/')

# About credentials
# There are 2 types of "credentials":
#   the one created and downloaded from https://console.developers.google.com/apis/
#   (let's call it the client_id)
#   and the one that will be created from the downloaded client_id (let's call it
#   credentials, it will be stored in C:\Users\user\.credentials)
...
def get_credentials():
    ...
    credential_path = os.path.join(credential_dir, 'r2d2_kp_client_id.json')
    ...
    if not credentials or credentials.invalid:
        CLIENT_SECRET_FILE = os.path.join(KEY_DIR, 'r2d2_kp_client_id.json')
        APPLICATION_NAME = 'r2d2-data-kp01'
        # The scope URL for sending mail from gmail
...
def create_message_and_send(service,
                            sender,
                            to,
                            subject,
                            message_text_plain,
    ...
    message_with_attachment = create_Message_with_attachment(
        sender, to, subject, message_text_plain, message_text_html, attached_file, f_name, dkht_logo_path)
...
def create_Message_with_attachment(sender, to, subject, message_text_plain, message_text_html, attached_file, f_name, dkht_logo_path):
    ...
    # The order count: the first (html) will be used for email, the second will be attached (unless you comment it)
    message.attach(MIMEText(message_text_html, 'html'))
    #message.attach(MIMEText(message_text_plain, 'plain'))

    with open(dkht_logo_path, 'rb') as fp:
    ...
    attachment = MIMEApplication(attached_file, _subtype='pdf')
    #     temp.close()
    #
    # else:
    ...
    # filename = os.path.basename('test_file.pdf')  # attached_file)
...
def send_email_report(recipient, subject, message_text_plain, message_text_html, attached_file, f_name, dkht_logo_path):
    sender = "dan@dkhydrotech.com"
    create_message_and_send(
        sender, recipient, subject, message_text_plain, message_text_html, attached_file, f_name, dkht_logo_path)
