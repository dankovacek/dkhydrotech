...
@lru_cache()
def load_data(filename, DA, ID, name):
    ...
    df = pd.DataFrame()
    df['Date'] = data['Date']
    df['daily_flow_' + name] = data['Value'].astype(float)
    df['daily_ur_' + name] = (data['Value'] * 1000 / DA).astype(float)
    df['log_ur_' + name] = np.log(df['daily_ur_' + name])
    df.set_index('Date', inplace=True)
    # df['date'] = df.index.astype(str)
    df = df.dropna(axis=0, how='any')
    # df = df.fillna(y.bfill())
    return df
...
def getRandomChronSets(p_train, p_val, p_test, data):
    """
    Takes as input:
    1. a dataframe "data" containing all
    features (xi) as well as the output vector (y).
    ...
    m = len(data.index.values)
    # order = np.random.permutation(m)
    p_val_start = int(m * (1 - (p_val + p_test)))
    p_test_start = p_val_start + int(p_val * m)

    train_data = data.ix[:p_val_start, :]
    val_data = data.ix[p_val_start:p_test_start, :]
    test_data = data.ix[p_test_start:, :]
    return train_data, test_data, val_data
